...
const BackgroundMobile =
  "/pages/about-us/cici-mandarin-team/OLD-PAPER-MOBILE.webp";
...
export default function CiciMandarinTeam() {
  ...
  return (
    <>
      <section
        className="bg-cover bg-center pt-14 pb-10 px-7 block md:hidden"
        style={{ backgroundImage: `url(${BackgroundMobile})` }}
      >
        <div className="flex flex-col gap-3 justify-center items-center">
          <div className="flex flex-col justify-center items-center gap-4 pt-5">
            <div className="text-center font-mochiy-pop-one font-bold text-xl text-[#CB0D0D]">
              CICI MANDARIN TEAM
            </div>
            <span className="text-center text-[#CB0D0D]">
              At Cici Mandarin, our strength is a team of professionals from
              Indonesia and China with firsthand experience{" "}
              <span className="font-bold ]">
                living, studying, and working in China.
              </span>{" "}
              We combine local insight with deep knowledge of China's culture,
              education, job market, and travel scene.
            </span>
            <span className="text-center text-[#CB0D0D]">
              What sets us apart is how we bridge both worlds—offering Mandarin
              lessons and personalized guidance{" "}
              <span className="font-bold ]">
                for study, work, and travel in China.
              </span>
  ...
  */}
            <div className="w-[15rem] h-[250px] flex overflow-x-auto snap-x snap-mandatory gap-4 rounded-xl shadow-lg z-20 pb-2 scroll-smooth">
              {videoList.map((videoSrc, index) => (
                <video
                  key={index}
                  className="min-w-full w-full h-full object-cover snap-center rounded-xl bg-black border-2 border-white"
                  controls
  ...
  */}
            <div className="flex gap-2">
              {videoList.map((_, index) => (
                <div
                  key={index}
                  className="w-2 h-2 rounded-full bg-red-600 opacity-50"
                ></div>
              ))}
              <span className="text-xs text-[#CB0D0D] ml-2 font-bold animate-pulse">
                &lt;&lt; Swipe &gt;&gt;
              </span>
            </div>
          </div>
      ...
      <section
        className="hidden md:block bg-cover bg-center pt-14 pb-10 px-7"
        style={{ backgroundImage: `url(${Background})` }}
      >
        <div className="flex flex-col gap-3 justify-center items-center">
          <div className="flex flex-col justify-center items-center gap-8 pt-10">
            <div className="text-center font-mochiy-pop-one font-bold text-3xl text-[#CB0D0D] pt-12">
              CICI MANDARIN TEAM
            </div>
              ...
              */}
              <div className="absolute top-[18%] right-[16%] w-[19rem] h-[74%] z-20 flex flex-col items-center">
                ...
                {/* Tips Styling:
                    ...
                    - object-cover: Memotong video jika rasionya beda, biar pas di kotak (Gak gepeng).
