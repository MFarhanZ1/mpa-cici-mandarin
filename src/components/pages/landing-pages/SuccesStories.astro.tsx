...
const videos = [
  "https://www.youtube.com/embed/vWz9HQPHpD8",
  "https://www.youtube.com/embed/FbWldMUwmAc",
  "https://www.youtube.com/embed/-GjR09l0OVo",
  "https://www.youtube.com/embed/XVwhGTO-eNc",
  "https://www.youtube.com/embed/msSJHQjvGtw",
  "https://www.youtube.com/embed/m17MXS-6UA4",
  "https://www.youtube.com/embed/cNXjF50su_A",
];
...
const textVideos = [
	...
	"Adult Mandarin Learning Fun",
	"Kids Mandarin Learning Fun", 
	"Preschoolers Class Fun", 
];
...
const backgroundImage = "pages/landing-pages/succes-stories/v3-success-stories.png";
const backgroundImageMobile = "pages/landing-pages/succes-stories/v4-success-stories-mobile.png";
...
const VideoCard: React.FC<VideoCardProps> = ({ videoSrc, logoSrc, isPlaying, onPlayToggle }) => {
  ...
  return (
    <div className="shrink-0 w-full h-full flex items-center justify-center">
      ...
      <div className="relative group cursor-pointer w-[74%] h-[60vh] rounded-2xl overflow-hidden bg-black shadow-lg" onClick={handleCardClick}>
        ...
        >
          <button type="button" className="play-button bg-[#CB0D0D] rounded-full hover:bg-[#a00a0a] transition-all transform hover:scale-110" style={{ padding: "14px 18px" }} aria-label="Play video" onClick={handlePlayClick}>
        ...
        {/* Mute/Unmute button */}
        <button type="button" className="mute-button absolute bottom-3 right-3 bg-black/70 hover:bg-black/90 rounded-full p-1.5 transition-all z-20" aria-label="Mute/Unmute video" onClick={handleMuteToggle}>
...
const SuccesStories: React.FC = () => {
  ...
  const handlePlayToggle = useCallback((index: number) => {
    setPlayingIndex((prevPlayingIndex) => (prevPlayingIndex === index ? null : index));
  }, []);
    ...
    <>
              ...
              >
                {videos.map((video, index) => (
                  <div key={video} className="w-full shrink-0">
                    <VideoCard videoSrc={video} logoSrc={ciciMandarinLogo} isPlaying={playingIndex === index} onPlayToggle={() => handlePlayToggle(index)} />
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex gap-2 mt-6">
              {videos.map((_, index) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all ${currentIndex === index ? "w-8 bg-white" : "w-2 bg-white/50 hover:bg-white/75"}`}
      ...
      */}
      <section className="overflow-hidden w-full h-full md:hidden relative">
        <img src={backgroundImageMobile} className="w-full h-full object-fit" alt="BG Image" />
        <img src={stampPNG} className="absolute top-[24vh] left-1 z-60 w-28 h-28" alt="Stamp" />
